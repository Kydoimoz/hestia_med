      ...
      <Container>
        <div className="grid max-w-screen-xl grid-cols-1 gap-10 pt-10 mx-auto mt-5 border-t border-gray-100 dark:border-trueGray-700 lg:grid-cols-5">
          <div className="lg:col-span-2">
            ...
            Hestia Med is a modern practice management system designed for general practitioners and private practices. It helps streamline patient management, appointment scheduling, and daily administrative tasks. Built with the latest technologies, it ensures secure data storage, scalability, and easy access.
            </div>
          </div>
          ...
          <div>
            <div className="flex flex-wrap w-full -mt-2 -ml-3 lg:ml-0">
              {navigation.map((item, index) => (
                <Link key={index} href="/" className="w-full px-4 py-2 text-gray-500 rounded-md dark:text-gray-300 focus:outline-none dark:focus:bg-trueGray-700">
                    {item}
                </Link>
              ))}
            </div>
          ...
          <div>
            <div className="flex flex-wrap w-full -mt-2 -ml-3 lg:ml-0">
              {legal.map((item, index) => (
                <Link key={index} href={item.value} className="w-full px-4 py-2 text-gray-500 rounded-md dark:text-gray-300 focus:outline-none dark:focus:bg-trueGray-700">
                    {item.label}
                </Link>
              ))}
            </div>
        ...
        <div className="hidden my-10 text-sm text-center text-gray-600 dark:text-gray-400">
          Copyright © {new Date().getFullYear()}. Made by{" "}
          <Link href="https://hestia-med.com/" target="_blank" rel="noopener">
            Hestia - Med
          </Link>{" "}
        </div>
      </Container>
